...
<img src="docs/images/hello.png" width="240" alt="HTGL rendering hello.html on the host simulator"/>

<sub><code>examples/hello.html</code> rendered by the C engine (host simulator → PNG)</sub>
...
→ transpiles to a **199-byte** `.uib` → renders to the image at the top of this README.
...
|---|---|---|
| **Header** | 16 B | magic `HTGL`, version, node count, screen W/H, string-table offset |
...
```sh
cd tool && python -m pytest      # transpiler (21 tests)
ctest --test-dir build           # engine + end-to-end golden image (6 tests)
```
...
| **M2** | STM32F1 + ILI9341 port (new `flush()`), engine core unchanged | ⏳ next |
| **M3** | Images, buttons + touch input (event dispatch) | 🔜 |
| **M4** | Flex/percent layout (fixed-point), animation | 🔜 |
